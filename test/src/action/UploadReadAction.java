...
public class UploadReadAction extends Action{
	...
	public String execute(HttpServletRequest request) {
		...
		map = fm.fileAction(list, map, session);
			// レビュー内容を取り出す
		l_revue = map.get("revue");
		if(l_revue.length() > 500) {
			// ※感想が500文字より多ければエラー
...
// ----------<< 画像を保存するフェイズ(画像がある場合だけ) >>----------
		// メソッドのために必要な引数を準備して、画像保存メソッドを起動する
		if(!baseItem.getString().equals("") && baseItem != null) {

			f_name = fm.imgWrite(baseItem, map.get("readName"), path);
		}
...
// ----------<< DBにインサートするフェイズ >>----------
		...
		int l_rs = ldao.deletePile(l_user.getU_id(), l_item.getI_id());

		// 完読データに追加
		l_revue = map.get("revue");
		ListDAO lldao = new ListDAO();
		l_rs = lldao.insertReadList(l_user.getU_id(), l_item.getI_id(), l_revue);
		if(l_rs == 0) {
			return "/error.jsp";
		}
...
// --------<< 遷移先に対しての処理 >>--------
		...
		ListDAO ldao = new ListDAO();
		p_list = ldao.searchPileAll(l_user.getU_id());
		...
		request.setAttribute("message", "完読を追加しました"); //JSPのログイン完了メッセージ
		return "/mypage.jsp";
	}
